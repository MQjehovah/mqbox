...
clipboard: {
  ...
  file: {
    ...
    listAudio: (dirPath: string) => ipcRenderer.invoke('files:list-audio', dirPath)
  },
screenshot: {
    ...
    pinMove: (dx: number, dy: number) => ipcRenderer.send('screenshot:pin-move-delta', { dx, dy }),
    pinResize: (width: number, height: number, mouseX: number, mouseY: number) => ipcRenderer.send('screenshot:pin-resize', { width, height, mouseX, mouseY }),
