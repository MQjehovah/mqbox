...
<template>
  <div class="search-box w-[680px] rounded-xl p-[16px] bg-[#FAFAFA] shadow-[0_4px_20px_#00000026]" tabindex="-1" @blur="hideWindow">
    <div class="search-input flex items-center gap-[12px] bg-white rounded-lg px-[12px] py-[8px] border border-[#E0E0E0]">
      <svg class="w-[20px] h-[20px] text-[#666666]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
      ...
      <input
        ...
        class="flex-1 outline-none text-[14px] text-[#1E1E1E]"
        @keydown="handleKeydown"
      />
      <svg class="w-[20px] h-[20px] text-[#666666] cursor-pointer" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" @click="hideWindow">
    ...
    <div v-if="!store.query" class="hint-area mt-[8px] flex items-center gap-[8px] bg-[#F5F5F5] rounded px-[12px] py-[8px]">
      ...
      <span class="text-[12px] text-[#666666]">输入 > 激活命令模式，输入 @插件名 直接调用插件</span>
    </div>
  </div>
</template>