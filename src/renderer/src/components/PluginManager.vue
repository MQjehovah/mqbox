    ...
    <div class="plugin-manager w-[500px] h-[460px] rounded-xl bg-white shadow-[0_4px_20px_#00000026] flex flex-col overflow-hidden">
      ...
      <div class="content flex-1 px-[20px] py-[8px] flex flex-col overflow-hidden">
        ...
        <template v-if="activeTab === 'shortcuts' && !selectedPluginId">
          <div class="list-header flex justify-between items-center pb-[8px] border-b border-[#E0E0E0]">
            <span class="text-[14px] text-[#666666]">快捷键绑定 ({{ shortcuts.length }})</span>
          ...
          <!-- Add form -->
          <div v-if="showAddShortcut" class="mt-[8px] bg-[#F5F5F5] rounded-lg p-[12px] flex flex-col gap-[8px]">
          ...
          <!-- Shortcut list -->
          <div class="flex-1 flex flex-col gap-[4px] mt-[8px] overflow-y-auto">
            <div v-for="(s, idx) in shortcuts" :key="s.accelerator + idx" class="flex items-center gap-[8px] py-[6px] px-[8px] rounded hover:bg-[#F9F9F9] group">
            ...
            <div v-if="shortcuts.length === 0" class="flex-1 flex items-center justify-center">
              <span class="text-[14px] text-[#999]">暂无快捷键，点击"添加"创建</span>
            </div>
          </div>
        </template>
